...
function Module:CreateMover(frame, name)
...
function Module:CreateMageBar(masqueGroup)
    local config = E.db[addonName].mageBar

    local polymorphs = {}
    for i, id in next, Module.database.Mage.PolymorphSheep do
    ...
    frame.buttons[1] = Module:CreateSpellFlyout("Teleports", frame, {
        ...
        direction = "UP",
        actions = Module.database.Mage.Teleports[faction],
        defaultActionIndex = 1,
        showOnlyMaxRank = false,
        masqueGroup = masqueGroup
    })
    frame.buttons[2] = Module:CreateSpellFlyout("Portals", frame, {
        size = config.buttonSize,
        direction = "UP",
        actions = Module.database.Mage.Portals[faction],
        defaultActionIndex = 1,
        showOnlyMaxRank = false,
        masqueGroup = masqueGroup
    })
    frame.buttons[3] = Module:CreateSpellFlyout("Water", frame, {
        size = config.buttonSize,
        direction = "UP",
        actions = Module.database.Mage.ConjureWater,
        defaultActionIndex = #Module.database.Mage.ConjureWater,
        showOnlyMaxRank = false,
        masqueGroup = masqueGroup
    })
    frame.buttons[4] = Module:CreateSpellFlyout("Food", frame, {
        size = config.buttonSize,
        direction = "UP",
        actions = Module.database.Mage.ConjureFood,
        defaultActionIndex = #Module.database.Mage.ConjureFood,
        showOnlyMaxRank = false,
        masqueGroup = masqueGroup
    })
    frame.buttons[5] = Module:CreateSpellFlyout("Gems", frame, {
        size = config.buttonSize,
        direction = "UP",
        actions = Module.database.Mage.ConjureGem,
        defaultActionIndex = #Module.database.Mage.ConjureGem,
        showOnlyMaxRank = false,
        masqueGroup = masqueGroup
    })
    frame.buttons[6] = Module:CreateSpellFlyout("Polymorph", frame, {
        size = config.buttonSize,
        direction = "UP",
        actions = polymorphs,
    ...
    frame:HookScript("OnEvent", function()
        Module:UpdateClassBar(frame)
    end)

    -- create ElvUI mover
    Module:CreateMover(frame, "MageBar")
    Module:UpdateClassBar(frame)

    -- create shadow
    if _G["ElvUI_Shadows"] then
        _G["ElvUI_Shadows"]:CreateShadow(frame)
    end

    return frame
end

function Module:CreateShamanBar(masqueGroup)
    local config = E.db[addonName].shamanBar

    local fire = {}
    for i, totem in next, Module.database.Shaman.FireTotems do
        for j, id in next, totem do
            table.insert(fire, id)
        end
    end
    local earth = {}
    for i, totem in next, Module.database.Shaman.EarthTotems do
        for j, id in next, totem do
            table.insert(earth, id)
        end
    end
    local water = {}
    for i, totem in next, Module.database.Shaman.WaterTotems do
        for j, id in next, totem do
            table.insert(water, id)
        end
    end
    local air = {}
    for i, totem in next, Module.database.Shaman.AirTotems do
        for j, id in next, totem do
            table.insert(air, id)
        end
    end
    local enchants = {}
    for i, totem in next, Module.database.Shaman.WeaponEnchants do
        for j, id in next, totem do
    ...
    frame.buttons[2] = Module:CreateSpellFlyout("EarthTotems", frame, {
        size = config.buttonSize,
        direction = "UP",
        actions = earth,
        defaultActionIndex = 1,
    ...
    frame.buttons[3] = Module:CreateSpellFlyout("WaterTotems", frame, {
        size = config.buttonSize,
        direction = "UP",
        actions = water,
        defaultActionIndex = 1,
    ...
    frame.buttons[4] = Module:CreateSpellFlyout("AirTotems", frame, {
        size = config.buttonSize,
        direction = "UP",
        actions = air,
        defaultActionIndex = 1,
    ...
    frame.buttons[5] = Module:CreateSpellFlyout("WeaponEnchants", frame, {
        size = config.buttonSize,
        direction = "UP",
        actions = enchants,
        defaultActionIndex = 1,
    ...
    frame:HookScript("OnEvent", function()
        for index = 1, MAX_TOTEMS do
            ...
            if (haveTotem and duration > 0) then
                ...
                local totemId = Module:FindTotem(totemName)
                if totemId then
    ...
    -- create ElvUI mover
    Module:CreateMover(frame, "ShamanBar")
    Module:UpdateClassBar(frame)
...
function Module:UpdateClassBar(bar)
    if not bar then
        return
    end
    ...
    for i, button in next, bar.buttons do
        Module:UpdateSpellFlyout(button)
...
function Module:CreateSpellFlyout(name, parent, config)
    ...
    button.CurrentAction:HookScript("OnEnter", function()
        ...
        Module:UpdateSpellFlyoutBackground(button)
    ...
    Module:FixNormalTextureSize(button.CurrentAction)
    ...
    button:SetScript("OnEvent", function(self, event)
        ...
        for i, child in next, button.childButtons do
            Module:UpdateSpellFlyoutChild(button, child)
        end
    end)
...
function Module:UpdateSpellFlyout(button)
    ...
    if not InCombatLockdown() then
        ...
        for i, action in next, actions do
            ...
            if isKnown then
                local shouldBeVisible = not button.showOnlyMaxRank or (button.showOnlyMaxRank and Module:IsMaxKnownRank(action))
                if not actionButton and shouldBeVisible then
                    actionButton = Module:CreateSpellFlyoutChild(button, action, i, previousButton)
        ...
        if button.showOnlyMaxRank then
            local maxRank = Module:GetMaxKnownRank(defaultAction)
            if maxRank then
...
function Module:UpdateSpellFlyoutBackground(button)
    button.isOpen = button.CurrentAction:GetAttribute("open") == 1
...
function Module:CreateSpellFlyoutChild(button, action, index, previousButton)
    ...
    Module:FixNormalTextureSize(actionButton)

    return actionButton
end

function Module:UpdateSpellFlyoutChild(button, child)
...
function Module:FixNormalTextureSize(button)
